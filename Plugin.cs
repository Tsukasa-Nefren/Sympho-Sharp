...
namespace Sympho
{
    ...
    public partial class Sympho : BasePlugin, IPluginConfig<Settings>
    {
        ...
        public override string ModuleVersion => "2.2.1 (Final & Stable)";
        ...
        private readonly ConcurrentQueue<(string url, int start, bool isAdmin)> _queue = new();
        private int _queueActive = 0;
        private CancellationTokenSource _cts = new();
        private readonly Audio.PlayEndHandler _onPlayEnd;
        
        private string _lastQueuedUrl = "";
        private CCSPlayerController? _lastQueuedPlayer = null;
        private DateTime _lastQueueTime = DateTime.MinValue;
        ...
        {
            _handler.Initialize(this, new AudioService { PluginDirectory = ModuleDirectory });
            _youtube.Initialize(this);
            _subs.Initialize(this);
            _cache.Initialize(this);
            
            // AudioHandler와 Subtitles 연결
        ...
        public void YoutubeCommand(CCSPlayerController? player, CommandInfo info)
        {
            ...
            if (string.IsNullOrWhiteSpace(argString)) { info.ReplyToCommand($" {Localizer["Prefix"]} Usage: {info.GetArg(0)} <url> [start]"); return; }
            ...
            EnqueueYoutubeFromChat(player, url, start, reply: s2 => info.ReplyToCommand(s2));
        }
        
        public void EnqueueYoutubeFromChat(CCSPlayerController? player, string url, int start, Action<string>? reply = null)
        {
            var now = DateTime.UtcNow;
            if (player != null && player.IsValid && player == _lastQueuedPlayer && url == _lastQueuedUrl && (now - _lastQueueTime).TotalSeconds < 2.0) return;
            _lastQueuedPlayer = player; _lastQueuedUrl = url; _lastQueueTime = now;
            if (!AllowPlaying) { reply?.Invoke($" {Localizer["Prefix"]} {Localizer["Audio.Disabled"]}"); return; }
            bool isAdmin = player == null || AdminManager.PlayerHasPermissions(player, "@css/kick");
            if (!Config.AllowYoutube && !(isAdmin && Config.BypassYoutubeAdmin)) { reply?.Invoke($" {Localizer["Prefix"]} {Localizer["Youtube.NotAllowed"]}"); return; }
            if (Config.EnableAntiSpam && AntiSpamData.GetCooldownLeft() > 0 && !isAdmin) { reply?.Invoke($" {Localizer["Prefix"]} {Localizer["Spam.Remaining", AntiSpamData.GetCooldownLeft().ToString("0.0")]}"); return; }
            if (Config.EnableAntiSpam && !isAdmin) AntiSpamData.SetPlayedCount(AntiSpamData.GetPlayedCount() + 1);
            if (Youtube.IsPlaying && !Config.QueueEnabled) { reply?.Invoke($" {Localizer["Prefix"]} {Localizer["Youtube.WaitForFinish"]}"); return; }
            if (Config.QueueEnabled && Config.MaxQueueLength > 0 && _queue.Count >= Config.MaxQueueLength) { reply?.Invoke($" {Localizer["Prefix"]} Queue is full."); return; }
            _queue.Enqueue((url, start, isAdmin));
            reply?.Invoke($" {Localizer["Prefix"]} Queued. Position: {_queue.Count}");
            if (Interlocked.CompareExchange(ref _queueActive, 1, 0) == 0) _ = ProcessQueueAsync();
        }
        ...
        public void Skip(CCSPlayerController? player, CommandInfo info)
        {
            ...
            Youtube.IsPlaying = false; 
            _cts.Cancel();
            Server.PrintToChatAll($" {Localizer["Prefix"]} Skipped.");
        }
        ...
        public void StopAllSound(CCSPlayerController? player)
        {
            ...
            if (Youtube.IsPlaying)
            {
                ...
                Youtube.IsPlaying = false;
                _cts.Cancel();
                Server.PrintToChatAll($" {Localizer["Prefix"]} Stopped all sounds and cleared the queue.");
            }
        }
        ...
        public void ToggleSympho(CCSPlayerController? player, CommandInfo info)
        {
            ...
            Server.PrintToChatAll($" {Localizer["Prefix"]} {(AllowPlaying ? "Enabled" : "Disabled")}");
        }

        [ConsoleCommand("css_reloadconfig", "Reload Sympho config")]
        public void ReloadConfig(CCSPlayerController? player, CommandInfo info) { LoadConfig(); Server.PrintToChatAll($" {Localizer["Prefix"]} Reloaded."); }
            ...
            try
            {
                while (_queue.TryDequeue(out var job))
                {
                    ...
                    try
                    {
                        ...
                        var audioPath = await _youtube.ProceedYoutubeVideo(job.url, job.start, endLimit);
                        if(audioPath == null) { Youtube.IsPlaying = false; continue; }
                        while (Youtube.IsPlaying && AllowPlaying && !_cts.Token.IsCancellationRequested)
                        {
                            await Task.Delay(500, _cts.Token);
                        }
                    }
                    catch (OperationCanceledException) { /* 스킵된 경우 정상 종료 */ }
                    finally
                    {
                        ...
                        if (Config.EnableAntiSpam && !job.isAdmin)
                        {
                            ...
                            {
                                AntiSpamData.SetCooldown(Server.CurrentTime + Config.AntiSpamCooldown);
